	...
	return (
		<div className="calPageContainer">
			<div className="calenderContainer">
				...
				<div className="weekDaysContainer">{weekDays.map((weekday) => <div>{weekday}</div>)}</div>
				<div className="daysContainer">
					{prevDays.map((day, i) => <Day key={i} day={day} />)}
					{daysArray.map((day, i) => <Day day={day} key={i} />)}
					{nextDaysArr.map((day, i) => <Day key={i} day={day} />)}
				</div>
			</div>
		</div>
	);
